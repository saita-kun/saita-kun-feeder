...
同一ホストで同じ台帳を使うランナーは、dry-run を含めて1プロセスだけ動作します。フィード取得前にロックを取得し、台帳読込み・送信・チャネルごとの結果保存が終わるまで保持します。競合した実行は待機・送信せず **exit 1** で終了し、ログに `台帳はロックされています` とロックの絶対パスを出します。通常の終了（exit 0/2）と処理例外（exit 1）では、自身が取得したロックを解放します。

ロックは正規化した台帳の実体パスに `.lock` を付けた空ディレクトリです（既定: `state/notified.json.lock/`）。相対・絶対パス、ファイルや親ディレクトリのシンボリックリンクは同じロックになります。台帳の保存先も実体パスに統一するため、リンクは維持されます。異なる台帳は同時処理できますが、同時実行する場合は `--out` も別にしてください。

**強制終了・クラッシュ後の明示復旧:** 残ったロックは、古さやPID不在だけでは自動回収しません。

1. cron・timer・手動起動など、その台帳を使う新規起動をすべて止めます。
2. 同じ台帳を使う全ランナーと、その子アダプタ・子孫プロセスがすべて終了したことを確認します。ランナーだけの終了確認では不十分です。確認できない場合はロックを残してください。
3. 送信後・保存前に停止した可能性があるため、配信先と台帳の状態を確認します。再実行で再送される可能性があります。
4. ログに示されたロックの絶対パスを使い、`rmdir -- /absolute/path/to/notified.json.lock` で空ディレクトリだけを除去します。失敗した場合は強制削除せず原因を確認します。
5. ランナーを1回実行して結果を確認し、定期起動を再開します。

実行中に台帳・リンクの付け替えやロックの削除をしないでください。ハードリンクの別名、複数ホスト・共有ネットワークファイルシステム、ラッパーのGit保存処理はこの排他の対象外です。
