...
1. **application_deadline**: 締切が既知なら `YYYY-MM-DD`。それ以外（`"No information"` 等の sentinel 文字列・null・非 ISO 形式）はすべて「**締切不明**」を意味する。consumer は「`^\d{4}-\d{2}-\d{2}$` に合致しなければ不明」として解釈しなければならない。締切不明の行は open 判定から保守的に除外される（vendored `parseDeadline` / `isOpen` の挙動が正）。
   - **暦として実在しない日付も「締切不明」として扱う（MUST）**。形式が `^\d{4}-\d{2}-\d{2}$` に合致していても、その年月日が暦上存在しない場合（`2027-02-29` — 2027 年は平年、`2026-04-31`、`2026-06-00` など）は不明と解釈し、生の文字列を締切として表示してはならない。producer は暦非実在日を検出したら `null` に落として公開する（行自体は落とさない）。consumer は producer の出力によらず、`^\d{4}-\d{2}-\d{2}$` 通過後に**暦日の往復検証**（パース結果の年月日3成分が入力の3成分と一致するか）を行わなければならない。ECMAScript の `Date` は非実在日の一部だけを Invalid Date にする。月の範囲外（`2026-13-01`・`2026-00-10`）と日が `00` や `32` 以上（`2026-06-00`・`2026-01-32`）は Invalid Date になるが、日が `29`〜`31` でその月に存在しない場合は翌月へ繰り上げられる（`2027-02-29` → `2027-03-01`、`2026-04-31` → `2026-05-01`）。したがって形式検査と `NaN` 検査だけでは不十分である。
