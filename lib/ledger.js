...
function setOwn(dictionary, key, value) {
  // Keep every data key enumerable, including "__proto__", after JSON loading.
...
function saveLedger(ledgerPath, ledger) {
  ...
  const temporary = path.join(dir, `.${path.basename(ledgerPath)}.${crypto.randomUUID()}.tmp`);
...
function planCandidate(ledger, subsidy, channelName, nowMs) {
  ...
  const entry = getOwn(ledger.entries, subsidy.id);
  if (!entry) return { action: 'new', hash };

  const ch = entry.channels && getOwn(entry.channels, channelName);
...
function recordResult(ledger, subsidy, channelName, { ok, nowIso, hash, notifiedAs }) {
  const entry = getOwn(ledger.entries, subsidy.id) || { channels: {} };
  entry.channels = entry.channels || {};

  const prev = getOwn(entry.channels, channelName);
  ...
  setOwn(entry.channels, channelName, {
  ...
  setOwn(ledger.entries, subsidy.id, entry);
}

/**
 * Collect subsidy IDs already sent (any channel) within [nowMs - windowMs, nowMs].
 * Used for weekly/daily budget calculation and resumed channel delivery.
 */
function sentIdsWithin(ledger, nowMs, windowMs) {
  const ids = new Set();
  for (const [id, entry] of Object.entries(ledger.entries)) {
    ...
    if (sentTimes.some((t) => nowMs - t <= windowMs && t <= nowMs)) ids.add(id);
  }
  return ids;
}

function countSentWithin(ledger, nowMs, windowMs) {
  return sentIdsWithin(ledger, nowMs, windowMs).size;
}
