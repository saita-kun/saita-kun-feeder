 ...
 * Diff detection uses a content hash over a FIXED field list (HASH_FIELDS):
 * changing that list re-notifies every row as "updated", so it only changes
 * together with the migration procedure described in dr-006.
 *
...
function planCandidate(ledger, subsidy, channelName, nowMs) {
  ...
  if (entry.content_hash !== hash) return { action: 'updated', hash };
...
function recordResult(ledger, subsidy, channelName, { ok, nowIso, hash, notifiedAs }) {
  const entry = ledger.entries[subsidy.id] || { content_hash: hash, channels: {} };
  ...
  const retryCount = ok ? 0 : (prev && prev.status === 'failed' ? (prev.retry_count || 0) + 1 : 0);
