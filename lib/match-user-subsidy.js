 ...
 * Nonexistent calendar dates also return null (unknown deadline).
 */
function parseDeadline(deadline) {
  ...
  if (
    d.getUTCFullYear() !== Number(deadline.slice(0, 4)) ||
    d.getUTCMonth() + 1 !== Number(deadline.slice(5, 7)) ||
    d.getUTCDate() !== Number(deadline.slice(8, 10))
  )
    return null;
  return d;
}
...
/**
 * Effective deadline for ranking only. fiscal_year stays NULL in storage and
 * sorts as the current Japanese fiscal-year end (March 31).
 */
function effectiveDeadlineForSort(subsidy, today = new Date()) {
  const row = subsidy || {};
  if (row.deadline_kind !== "fiscal_year") {
    return parseDeadline(row.application_deadline);
  }

  const current = today instanceof Date ? today : new Date(today);
  if (!Number.isFinite(current.getTime())) return null;
  const currentJst = new Date(current.getTime() + JST_OFFSET_MS);
  const endYear = currentJst.getUTCMonth() < 3 ? currentJst.getUTCFullYear() : currentJst.getUTCFullYear() + 1;
  return new Date(Date.UTC(endYear, 2, 31));
}
...
function matchesNonRegionCriteria(subsidy, criteria, opts = {}) {
  ...
  return userScaleMeetsSubsidy(userScale, subsidy.eligible_scale);
}
