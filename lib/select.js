 ...
 * budgets reserve these slots among rows that actually pass the budget.
...
function selectWithUrgentSlots(base, today, accept) {
  ...
  const take = (index) => {
    processed.add(index);
    const subsidy = base[index];
    if (!accept(subsidy)) return false;
  ...
  while (baseIndex < base.length) {
    ...
    if (out.length < URGENT_WINDOW) {
      ...
      if (need > 0 && slotsLeft <= need) {
        let promoted = false;
        while (urgentIndex < base.length) {
          const index = urgentIndex++;
          if (!processed.has(index) && isUrgent(base[index], today) && take(index)) {
            promoted = true;
            break;
          }
        }
        // Rejected urgent rows do not occupy slots. If none remain affordable,
        // continue in base order without consuming the missing reservation.
        if (promoted) continue;
      }
    }
    // The urgent scan may have rejected the current base row as well.
    if (!processed.has(baseIndex)) take(baseIndex);
    baseIndex += 1;
  }
  return out;
}
