 ...
 * row_count) and falls back to the last known-good cache on acquisition or
 * verification failure (dr-004: graceful degradation). Node stdlib only.
 *
...
function saveCache(cachePath, verified) {
  const dir = path.dirname(cachePath);
  fs.mkdirSync(dir, { recursive: true });
  const temporary = path.join(dir, `.${path.basename(cachePath)}.${crypto.randomUUID()}.tmp`);
  const fd = fs.openSync(temporary, 'wx');
  try {
    try {
      fs.writeFileSync(fd, JSON.stringify({ meta: verified.meta, data: verified.data }));
    } finally {
      fs.closeSync(fd);
    }
    ...
    try {
      ...
      // Cleanup is best-effort; a save failure must not discard verified data.
    }
    throw error;
  }
}
 ...
 * Cache save failures warn without discarding the verified feed.
 */
async function loadFeed({ baseUrl, cachePath }) {
  ...
  if (verified) {
    if (cachePath) {
      try {
        saveCache(cachePath, verified);
      } catch {
        warnings.push('キャッシュ保存に失敗しました（今回取得した検証済みフィードで継続します）');
      }
    }
