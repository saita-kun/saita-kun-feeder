...
async function fetchOptionalBytes(location) {
  try {
    return await fetchBytes(location);
  } catch {
    return null;
  }
}
...
async function fetchAndVerify(baseUrl) {
  ...
  let dataBytes;
  const gzBytes = await fetchOptionalBytes(joinFeedUrl(baseUrl, 'subsidies.json.gz'));
  if (gzBytes) {
