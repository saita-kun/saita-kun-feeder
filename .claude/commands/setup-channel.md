...
3. **実送信テスト**（**手元での実行だけで運用する場合**）: 利用者に「1 回だけテスト送信して良いか」を確認してから、golden digest fixture を実送信し、届いたことを利用者に確認してもらう。
   - **GitHub Actions で自動配信する場合は、ここを飛ばして手順 6 の手動実行を実送信テストとして使います。** 両方行うと契約 §6 の「利用者の確認を取ってから 1 回だけ」を超えて 2 回送ることになるためです。
...
3. push 後に Actions タブから deliver ワークフローを `Run workflow`（workflow_dispatch）で 1 回手動実行する。実行する ref は、日次実行と同じ**デフォルトブランチ**を選びます（`schedule` はデフォルトブランチ上の workflow とプロファイルしか使わないため、作業ブランチのままだと翌日以降は古い設定で動きます）。
   - **この手動実行は本物の配信で、Actions を使う利用者にとってはこれが唯一の実送信テストです**（だから手順 4-3 を飛ばします）。押す前に「実際の通知が 1 通届きますがよいか」を利用者に確認します（契約 §6）。

受入基準は「実行が緑」だけでは不足です。**実行ログの Deliver ステップに `[my-<name>] 送信成功` の行が出ていること**、さらに**期待した宛先に実際に届いたことを利用者に確認してもらうこと**まで揃って合格です（ログの `送信成功` はアダプタが exit 0 を返したという意味しかありません。送信先を誤っていても成功と記録され、台帳が `sent` で確定して再送されません）。新着・更新がなくフィード警告も無い日は、ランナーが `[my-<name>] 新着・更新なし — 配信しません` と出してアダプタを起動せずに終了します（＝緑になりますが `env:` の受け渡しは検証されていません）。その場合はその旨を利用者に伝え、翌日以降の実行ログと受信で改めて確認します。
